...
void laplacian(sampler2D icolor, out vec2 size, out vec4 l ){
    ...
    vec4 center = texelFetch( icolor, texelPos,0 ) ;
    float omega = 1./3. ;
    l = (1.-omega) *((
            texelFetch( icolor , unpack(N),0 )
        +   texelFetch( icolor , unpack(S),0 )
        +   texelFetch( icolor , unpack(E),0 )
        +   texelFetch( icolor , unpack(W),0 ) ) -4.*center) +
        omega*0.5*((
            texelFetch( icolor , unpack(NE),0 )
        +   texelFetch( icolor , unpack(NW),0 )
        +   texelFetch( icolor , unpack(SE),0 )
        +   texelFetch( icolor , unpack(SW),0 ) )- 4.*center) ;
